    ...
    public void onItemClick(AdapterView<?> parent, View view, int position,
        ...
        if (object instanceof Cursor) {
            ...
            if (!new File(path).exists()||path.length()==0){
                Toast.makeText(getActivity(), "视频不存在，请选择其他视频！", Toast.LENGTH_SHORT).show();
                return;
            }
...
//            }
            MediaPickerActivity activity=(MediaPickerActivity)getActivity();
            activity.returnVideo((getMediaSelectedList().get(0).getUriOrigin()));
        }
    }
